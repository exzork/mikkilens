...
const (
	...
	melContext    = 480 // extra left context, so the new frames have history
	embeddingSize = 96
...
func findRuntimeLibrary() (string, error) {
	...
	return "", &Error{Reason: "the ONNX runtime library was not found; put onnxruntime.dll " +
		"in data/models to use a wake word"}
}

// model wraps one ONNX session with its input and output names.
type model struct {
...
func (m *model) run(shape ort.Shape, data []float32) ([]float32, error) {
	...
	// A nil output lets onnxruntime allocate whatever shape the model produces,
	// which keeps this working across openWakeWord's model revisions.
